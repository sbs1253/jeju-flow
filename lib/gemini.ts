...
export async function generateUnifiedInsight(
  ...
  try {
    ...
    const prompt = `
      ...
      [작성 지침 - 중요]
      1. **타겟 준수**: 분석 대상이 '${targetAges}'인 경우, 특정 세대(예: 2030대)에만 국한된 이야기를 하지 마세요. 전체 방문객을 아우르는 포괄적인 시각을 유지하고, 헤드라인에서 특정 나이대를 언급하지 마세요.
      2. **수요-공급 격차 분석**: 사람들이 많이 검색하는 키워드(수요)와 실제 진행 중인 공연(공급)을 비교하여, 현재 부족하거나 유망한 분야를 짚어주세요.
      3. 모든 인사이트는 위에서 명시한 타겟 그룹의 실제 라이프스타일에 맞춤화되어야 합니다.
      4. **jeju_insights**: 구체적인 문화 기획 아이디어를 6-8개 제안하세요. 각 아이디어는 '왜 지금 이 트렌드에 필요한지' 근거를 포함해야 합니다.
      5. **trending_keywords**: 현재 트렌드와 연관된 공연/문화 키워드 10개 이상을 생성하세요.
      6. 실현 가능한 수준의 전문적인 톤을 유지하세요.
    `;
