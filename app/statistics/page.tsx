  ...
  const stats = useMemo(() => {
    ...
    filteredPerformances.forEach((p: Performance) => {
      ...
      if (regionFilter === 'jeju') {
        ...
        const isSeogwipo =
          venueName.includes('서귀포') ||
          venueName.includes('남원') ||
          venueName.includes('성산') ||
          venueName.includes('표선') ||
          venueName.includes('대정') ||
          venueName.includes('안덕');
            ...
            <div className="flex items-center gap-6 w-full md:w-auto">
              <Tabs value={regionFilter} onValueChange={setRegionFilter} className="w-full md:w-auto">
                <TabsList className="bg-white/5 p-1 rounded-2xl h-12 border border-white/5">
                  <TabsTrigger
                    value="all"
                    className="rounded-xl px-6 font-bold text-[11px] data-[state=active]:bg-sky-500 data-[state=active]:text-white"
                  >
                    전국
                  </TabsTrigger>
                  <TabsTrigger
              ...
              <Tabs value={statusFilter} onValueChange={setStatusFilter} className="w-full md:w-auto overflow-x-auto">
                <TabsList className="bg-white/5 p-1 rounded-2xl h-12 border border-white/5">
                  <TabsTrigger
                    value="all"
                    className="rounded-xl px-4 font-bold text-[11px] data-[state=active]:bg-sky-500 data-[state=active]:text-white"
                  >
                    전체
                  </TabsTrigger>
                  <TabsTrigger
                  ...
                  <div className="relative bg-card/80 backdrop-blur-2xl border border-primary/20 rounded-[2rem] p-8 overflow-hidden">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground font-medium">
                          검색어 수요 vs 실제 공연 공급 매칭 리포트 (최근 60일 데이터 기준)
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      <div className="lg:col-span-2 space-y-4">
                        ...
                        <p className="text-muted-foreground leading-relaxed">{insightData.weekly_summary.summary}</p>
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 auto-rows-fr">
                  ...
                  <StatCard
                    title="최다 장르"
                    value={
                      (Object.entries(stats.byGenre) as [string, number][]).sort((a, b) => b[1] - a[1])[0]?.[0] || '-'
                    }
                    icon={BarChart3}
                    color="lava"
                  />
                  <StatCard title="수집 업데이트" value="Live" icon={Calendar} color="forest" />
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
                  ...
                  <div className="flex flex-col gap-6 h-full">
                    ...
                    {regionFilter === 'jeju' && Object.keys(stats.byVenue).length > 0 && (
                      <CategoryBarChart data={stats.byVenue} title="제주 인기 공연장 (TOP 5)" height={280} />
                    )}
                  </div>
                </div>
              </>
            ...
            <div className="space-y-6">
              ...
              {filteredPerformances.length > 40 && (
                <div className="flex justify-center pt-8">
                  <p className="text-xs text-muted-foreground font-medium">최근 40건의 데이터만 표시 중입니다.</p>
                </div>
              )}
            </div>
          </div>
